...
logger = logging.getLogger('signals')  # لاگر اختصاصی برای سیگنال‌ها
...
def connect_car_to_document(sender, instance, created, **kwargs):
    if getattr(instance, '_signal_handled', False):
        return
    def _connect_car():
        logger.info(f"سیگنال CarEntry اجرا شد برای {instance.acceptance_number}")
        if instance.engine_number and instance.chassis_number:
            ...
            with transaction.atomic():
                if current_doc and (
                    ...
                    current_doc.car = None
                    current_doc._from_signal = True
                    current_doc.save(update_fields=['car'])
                    instance.document = None
                    instance.save(update_fields=['document'])
                    logger.info("ارتباط با سند قبلی قطع شد")
                ...
                if not instance.document:
                    try:
                        ...
                        doc._signal_handled = True
                        instance._from_signal = True
                        doc._from_signal = True
                        instance.save(update_fields=['document'])
                        doc.save(update_fields=['car'])
                        logger.info(f"سند {doc.id} به خودرو {instance.acceptance_number} وصل شد")
                    except Document.DoesNotExist:
                        logger.warning(f"سندی با این مشخصات پیدا نشد - engine_number={instance.engine_number}, chassis_number={instance.chassis_number}")
                    except Document.MultipleObjectsReturned:
                        logger.error(f"چند سند با این مشخصات پیدا شد - engine_number={instance.engine_number}, chassis_number={instance.chassis_number}")
    transaction.on_commit(_connect_car)
...
def connect_document_to_car(sender, instance, created, **kwargs):
    ...
    def _connect_document():
        logger.info(f"سیگنال Document اجرا شد برای سند {instance.id}")
        if instance.engine_number and instance.chassis_number:
            ...
            with transaction.atomic():
                if current_car and (
                    ...
                    current_car.document = None
                    current_car._from_signal = True
                    current_car.save(update_fields=['document'])
                    instance.car = None
                    instance.save(update_fields=['car'])
                    logger.info("ارتباط با خودرو قبلی قطع شد")
                ...
                if not instance.car:
                    try:
                        ...
                        instance.car = new_car
                        new_car.document = instance
                        instance._signal_handled = True
                        new_car._signal_handled = True
                        instance._from_signal = True
                        new_car._from_signal = True
                        instance.save(update_fields=['car'])
                        new_car.save(update_fields=['document'])
                        logger.info(f"سند {instance.id} به خودرو {new_car.acceptance_number} وصل شد")
                    except CarEntry.DoesNotExist:
                        logger.warning(f"خودرویی با این مشخصات پیدا نشد - engine_number={instance.engine_number}, chassis_number={instance.chassis_number}")
                    except CarEntry.MultipleObjectsReturned:
                        logger.error(f"چند خودرو با این مشخصات پیدا شد - engine_number={instance.engine_number}, chassis_number={instance.chassis_number}")
